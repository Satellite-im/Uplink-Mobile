...
class WarpBloc extends Bloc<WarpEvent, WarpState> {
  WarpBloc() : super(WarpStateInitial()) {
    on<EnableWarp>((event, emit) async {
      try {
        ...
        await _checkIfTesseractExists('1234');
        multipass =
            warp_mp_ipfs.multipass_ipfs_persistent(tesseract, _multipassPath!);
  ...
  Future<bool> _checkIfTesseractExists(String passphrase) async {
    try {
      tesseract = warp.Tesseract.fromFile(_tesseractPath!);
  ...
  late warp_multipass.MultiPass multipass;
